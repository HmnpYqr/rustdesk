...
static FILEDESCRIPTORW *wf_cliprdr_get_file_descriptor(WCHAR *file_name, size_t pathLen)
{
	...
	//
	// Only claim a size we actually have. GetFileSize() reports failure as
	// INVALID_FILE_SIZE, which cannot be told apart from a genuine 4GB-1 file without
	// GetLastError(), and it fails outright on the directory handles FILE_FLAG_BACKUP_SEMANTICS
	// lets us open above. Leaving the flag off in either case keeps the old behaviour for
	// that entry rather than announcing a length the receiver would then read against.
