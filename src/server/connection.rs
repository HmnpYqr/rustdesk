...
/// holds its share, or when the global limit is reached: then the oldest connection not yet
/// told to go is told to, and this one is refused rather than let in on a place that is still
/// occupied. The controller retries on its own.
pub fn admit_unauthorized(id: i32, ip: IpAddr) -> Option<UnauthorizedID> {
    ...
    if conns.len() >= MAX_UNAUTHORIZED_CONNS {
        if let Some((_, _, shared)) = conns
            .iter()
            .find(|(_, _, shared)| !shared.evicted.load(Ordering::Acquire))
        {
            shared.evicted.store(true, Ordering::Release);
...
mod test {
    ...
    async fn test_unauthorized_full_tells_the_oldest_to_go_and_frees_its_place_only_when_it_has() {
        ...
        // A further arrival tells the next oldest to go, never one already told.
        assert!(admit_unauthorized(2_001_000, "198.51.100.251".parse().unwrap()).is_none());
        assert!(held[1].is_evicted());
        assert!(!held[2].is_evicted());
    ...
    async fn test_unauthorized_limit_bounds_live_handshakes() {
        ...
        drop(controllers);
        for h in handshakes {
            let _ = hbb_common::timeout(3000, h).await;
        }
    }
